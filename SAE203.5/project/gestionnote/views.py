...
def generate_report(request, student_id):
    try:
        # Get student information and scores from the database based on student ID
        student = Etudiant.objects.get(Netudiant=student_id)
        scores = Note.objects.filter(etudiant=student)
